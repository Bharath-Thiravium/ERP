...
interface DirectCreateInvoiceModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
}

const DirectCreateInvoiceModal: React.FC<DirectCreateInvoiceModalProps> = ({
  isOpen,
  onClose,
  onSuccess
}) => {
  ...
  const handleCustomerChange = (customerId: string) => {
    ...
    // Determine GST type based on customer GSTIN
    if (customer?.gstin) {
      const customerStateCode = customer.gstin.substring(0, 2)
      // Assuming company is in Maharashtra (27) - you can make this dynamic
  ...
  const calculateTax = () => {
    if (gstType === 'exempt') return 0
    
    return items.reduce((totalTax, item) => {
      const product = products.find(p => p.id === item.product)
      if (product && item.line_total > 0) {
        const itemTax = (item.line_total * product.gst_rate) / 100
  ...
  const calculateTotal = () => {
    ...
    return subtotal - discount + tax + formData.shipping_charges + formData.other_charges
  }
                  ...
                  <div className="col-span-2">
                    ...
                    {item.product > 0 && (
                      <div className="text-xs text-gray-500 mt-1">
                        GST: {products.find(p => p.id === item.product)?.gst_rate || 0}%
                      </div>
                    )}
                  </div>
            ...
            <div className="space-y-2">
              ...
              {gstType !== 'exempt' && (
                <div className="flex justify-between text-sm">
                  <span>Tax ({gstType === 'cgst_sgst' ? 'CGST + SGST' : 'IGST'}):</span>
              ...
              {selectedCustomer && (
                <div className="text-xs text-gray-500 mt-2">
                  GST Type: {gstType === 'cgst_sgst' ? 'Intra-State (CGST + SGST)' : 
                           gstType === 'igst' ? 'Inter-State (IGST)' : 'Tax Exempt'}
                  {selectedCustomer.gstin && (
                    <span className="ml-2">| Customer GSTIN: {selectedCustomer.gstin}</span>
                  )}
                </div>
              )}
            </div>
...
export default DirectCreateInvoiceModal