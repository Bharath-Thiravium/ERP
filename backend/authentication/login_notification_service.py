...
class LoginNotificationService:
    ...
    def get_notification_email(master_admin):
        ...
        try:
            settings = SecuritySettings.objects.get(master_admin=master_admin)
            # Use custom notification email if set, otherwise use account email
            return settings.notification_email or master_admin.user.email
