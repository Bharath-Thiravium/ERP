...
def validate_gstin(request):
    ...
    result = gst_service.validate_gstin(gstin)
...
def validate_pan(request):
    ...
    result = tds_service.validate_pan(pan)
    return Response(result)
...
def get_gst_rates(request):
    ...
    rates = gst_service.get_gst_rates(hsn_code)
    if rates:
...
def get_tds_rates(request):
    ...
    rates = tds_service.get_tds_rates(section_code, assessment_year)
    if rates:
        return Response(rates)
    else:
...
def file_gstr1(request):
    ...
    result = gst_service.file_gstr1(gstin, return_period, invoice_data)
    return Response(result)
...
def file_tds_return(request):
    ...
    for payment in payments:
        if payment.tds_amount:
            deductees.append({
                'pan': payment.invoice.customer.pan,
    ...
    result = tds_service.file_tds_return(quarter, financial_year, tds_data)
    return Response(result)
...
def generate_einvoice(request):
    ...
    try:
        invoice = Invoice.objects.get(id=invoice_id, company=request.user.company)
    ...
    invoice_data = {
        ...
        'SellerDtls': {
            'Gstin': request.user.company.gstin,
            'LglNm': request.user.company.name,
            'Addr1': request.user.company.address,
            'Loc': request.user.company.city,
            'Pin': request.user.company.pincode,
            'Stcd': request.user.company.state_code
        },
        'BuyerDtls': {
    ...
    result = einvoice_service.generate_irn(invoice_data)
...
def bulk_validate_customers(request):
    ...
    for customer in customers:
        ...
        if customer.gstin:
            gst_result = gst_service.validate_gstin(customer.gstin)
            result['gstin_validation'] = gst_result
        
        # Validate PAN if present
        if customer.pan:
            pan_result = tds_service.validate_pan(customer.pan)
