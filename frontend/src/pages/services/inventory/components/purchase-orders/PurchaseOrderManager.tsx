...
const PurchaseOrderManager: React.FC = () => {
  ...
  const handleStatusChange = async (order: PurchaseOrder, newStatus: string) => {
    try {
      await inventoryApi.updatePurchaseOrder(order.id, { status: newStatus });
                  ...
                  <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedOrder.status === 'draft' && (
                        <button onClick={() => handleStatusChange(selectedOrder, 'pending')}
                          className="px-4 py-2 bg-yellow-500 text-white rounded-lg text-sm font-medium hover:bg-yellow-600">
                          📤 Send to Supplier
                        </button>
                      )}
                      {selectedOrder.status === 'pending' && (
                        <button onClick={() => handleStatusChange(selectedOrder, 'approved')}
                          className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm font-medium hover:bg-blue-600">
                          ✅ Confirm Order
                        </button>
                      )}
                      {(selectedOrder.status === 'approved' || selectedOrder.status === 'ordered') && (
                        <button onClick={() => handleStatusChange(selectedOrder, 'received')}
                          className="px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-600">
                          📦 Mark as Received
                        </button>
                      )}
                      {selectedOrder.status !== 'cancelled' && selectedOrder.status !== 'received' && (
                        <button onClick={() => handleStatusChange(selectedOrder, 'cancelled')}
