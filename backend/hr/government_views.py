...
def submit_to_government_portal(request):
    """Submit return to government portal"""
...
def get_submission_history(request):
    ...
    try:
        ...
        # Get submission logs
        submissions = SubmissionLog.objects.filter(company=company).order_by('-created_at')
        
        # Filter by portal if specified
        portal_name = request.query_params.get('portal_name')
        if portal_name:
            submissions = submissions.filter(portal_name=portal_name.upper())
...
def get_challans(request):
    ...
    try:
        ...
        # Get challans
        challans = ChallanGeneration.objects.filter(company=company).order_by('-created_at')
